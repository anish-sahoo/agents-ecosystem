...
access: read-only
model_tier: high
description: High-context second opinion on direction and decisions. Challenges assumptions, catches drift between the current trajectory and earlier decisions, and recommends the safest next move. Read-only advisory — never edits. Use before risky or ambiguous changes, when stuck, or when a decision feels consequential.
---
