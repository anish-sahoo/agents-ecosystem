...
access: read-only
model_tier: high
description: Adversarial reasoning reviewer — finds logic errors, weak explanations, unsupported claims, and gaps that mechanical checklists miss. Read-only — reports issues, never fixes them. Use to pressure-test an analysis, plan, conclusion, or explanation (not for style or line-level code review).
---
