/**
 * notify — opencode example plugin
 *
 * Native OS + cmux notifications when the agent needs a human back.
 * Other agent runtimes: adapt hooks/events; keep the cmux CLI helpers.
 *
 * Philosophy: "Notify the human when the AI needs them back, not for every micro-event."
 *
 * - cmux notify + set-status/clear-status when inside cmux
 * - Desktop fallback: macOS alerter, Windows SnoreToast, Linux notify-send
 * - Suppress when terminal focused (macOS); parent sessions only by default
 */
...
// @ts-expect-error - peer dep; install detect-terminal alongside the plugin
import detectTerminal from "detect-terminal"
// @ts-expect-error - peer dep; install node-notifier alongside the plugin
...
async function loadConfig(): Promise<NotifyConfig> {
	const configPath = path.join(os.homedir(), ".config", "opencode", "notify.json")
