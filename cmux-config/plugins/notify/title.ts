...
/** Optional JSON env from a launcher that owns the terminal title. */
const TITLE_CONTEXT_ENV_KEY = "AGENT_TITLE_CONTEXT"
...
export function parseOscTitleContext(
	...
	const rawContext = toNonEmptyString(env[TITLE_CONTEXT_ENV_KEY])
	if (!rawContext) {
		return null
	}
